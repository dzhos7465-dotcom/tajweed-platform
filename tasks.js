...
const TASK_WEIGHTS = {
  ...
  find:   6,    /* найди в аяте: место + правило, без вариантов на выбор —
                   самое ценное задание теста и потому самое дорогое.
                   Было 10; при трёх таких аятах они забирали треть теста,
                   и оценка начинала зависеть от трёх заданий: одна помарка
                   роняла результат непропорционально знанию. Шесть держит
                   его первым по весу, не отдавая ему работу целиком. */
  recite: 5,    /* чтение вслух — РУЧНАЯ часть. В автоматический балл не
                   входит: проверка возвращает его как «ждёт преподавателя».
                   Чтение даёт свои 20% итога отдельным каналом, поэтому
                   число здесь на оценку не влияет и служит только мерой
                   ручной части. */
};

/* ── ДОЛИ ВИДОВ ЗАДАНИЙ ───────────────────────────────────────────────
   Оценка делится между ВИДАМИ заданий, а не между самими заданиями.
   Так устроены школьные и университетские журналы: у контрольных своя
   доля, у домашних своя, и она не зависит от того, сколько их задали.

   Зачем так. Пока балл складывался по заданиям, доля вида зависела от их
   числа: поставил десять вопросов и один «найди» — вопросы решили работу,
   хотел того преподаватель или нет. Теперь он назначает, ЧТО проверяет,
   а сколько заданий на это уйдёт — дело удобства.

   Числа — относительная трудность вида. Их не надо складывать до ста:
   платформа берёт только те виды, что есть в работе, и делит между ними
   доступные баллы по этим числам.

     найди в аяте        5   ученик сам находит место, выбора вариантов нет
     распределение       4   решение по каждому слову, угадать трудно
     вопрос о правиле    4   выбор из четырёх, зато проходит ВСЕ темы

   Почему вопрос сравнялся с распределением. Он единственный вид, который
   обходит правила подряд — все, какие есть в программе. Распределение
   берёт глубиной внутри одного раздела, вопрос берёт широтой. При тройке
   выходило, что вопросы по всем правилам таджвида весят столько же,
   сколько буквы нулевого курса, — ступень ниже сравнялась со ступенью
   выше, и это было неверно.

   ВНУТРИ вида задания равны между собой. Три вопроса или тридцать — доля
   вида та же, просто делится на большее число.

   ЧЕГО ЭТО СТОИТ. Если в виде всего одно задание, оно унесёт всю долю
   вида, и случайная ошибка обойдётся дорого. Панель предупреждает, когда
   заданий в виде меньше трёх. */
...
/* ── НУЛЕВОЙ КУРС В СМЕШАННОЙ РАБОТЕ ──────────────────────────────────
   У него не относительная доля, а ТВЁРДАЯ: 15% теста, то есть 12 баллов
   из ста, когда в работе есть чтение.

   Было 20% — и выходило 16 баллов, ровно столько же, сколько за вопросы
   по всем правилам таджвида. Ступень ниже не должна стоить как ступень
   выше: теперь буквы дешевле вопросов примерно вдвое.

   Почему не как у остальных. Относительная доля скачет от того, какие
   ещё виды попали в работу: рядом с одними вопросами нулевой курс забирал
   бы 40 баллов, рядом с полным набором — 6. Преподавателю нужно другое:
   чтобы буквы всегда занимали своё скромное место, а работа оставалась
   контрольной по правилам.

   Если правил в работе нет вовсе, нулевой курс забирает тест целиком —
   это уже контрольная по буквам, и делить не с кем. */
const COURSE0_TEST_SHARE = 0.15;

/* К какому виду относится задание. Чтение сюда не входит: оно ручное и
   даёт свои 20 баллов итога отдельно от теста. */
function categoryOfTask(task) {
  if (!task) return null;
  if (task.type === TASK_TYPES.RECITE) return null;
  if (task.stage === 'course0') return 'course0';
  if (task.type === TASK_TYPES.FIND) return 'find';
  if (task.type === TASK_TYPES.SORT) return 'sort';
  return 'single';
}

/* ── ВЕС ЗАДАНИЙ НУЛЕВОГО КУРСА ───────────────────────────────────────
   Нулевой курс — ступень ниже. Когда в одной работе сходятся буквы и
   правила, вопрос о букве не должен стоить столько же, сколько вопрос о
   правиле: правило и труднее, и важнее.

   ПРАВИЛО ПРОСТОЕ: любое задание нулевого курса весит МЕНЬШЕ, чем самое
   дешёвое задание первого. Самое дешёвое там — вопрос о правиле, он стоит
   3. Значит потолок нулевого курса — ниже трёх.

   Вопрос о букве — 2. Распределение — 2,5: внутри шесть решений, и
   приравнивать его к одному вопросу было бы несправедливо, но и до
   правила оно не дотягивает. Дробный вес платформе не мешает: итог всё
   равно считается долей набранного от возможного.

   ЧЕГО ВЕСА НЕ ДЕЛАЮТ. Итог считается как доля набранного от возможного,
   поэтому в работе ИЗ ОДНИХ букв веса не меняют ничего: хоть 2, хоть 20 —
   процент тот же. Они важны только там, где ступени смешаны. И даже там
   главный рычаг не вес, а КОЛИЧЕСТВО: тридцать вопросов о буквах при пяти
   о правилах решат оценку, какой вес им ни ставь. */
const COURSE0_WEIGHTS = {
  single: 2,
  sort:   2.5,
};
