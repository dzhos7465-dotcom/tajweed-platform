...
(function () {
  ...
  function advice(review, r) {
    ...
    if (reciteTasks.length) {
      ...
      if (avg != null) {
        /* ЧТО ЗДЕСЬ МОЖНО ГОВОРИТЬ, А ЧТО НЕЛЬЗЯ.
           Балл за чтение ставит преподаватель на слух; платформа не знает,
           ЧТО именно прозвучало не так — гунна, длительность, буква. Поэтому
           советов по таджвиду тут быть не должно: это область преподавателя,
           и выдумывать их за него нельзя.

           Говорим только о том, что следует из чисел: уровень, ровность
           чтения между аятами и соотношение с теорией. Оттого и текст у
           разных учеников выходит разный — различаются сами факты. */
        const hi = Math.max.apply(null, grades), lo = Math.min.apply(null, grades);
        const spread = (grades.length > 1) ? (hi - lo) : 0;

        let txt;
        if (avg >= 9.5)      txt = '<b>Чтение отличное</b> — ' + fmtPts(avg) + ' из 10.';
        else if (avg >= 8.5) txt = '<b>Чтение очень хорошее</b> — ' + fmtPts(avg) + ' из 10.';
        else if (avg >= 7)   txt = 'Чтение уверенное — ' + fmtPts(avg) + ' из 10.';
        else if (avg >= 5)   txt = 'Чтение нужно подтянуть — ' + fmtPts(avg) + ' из 10.';
        else                 txt = '<b>Над чтением нужно поработать</b> — ' + fmtPts(avg) + ' из 10.';

        // ровность: одинаково ли даются разные аяты
        if (spread >= 3) {
          txt += ' Читаешь неровно: один аят на ' + fmtPts(hi) + ', другой на ' + fmtPts(lo) +
                 ' — разбери с преподавателем тот, что дался хуже.';
        } else if (grades.length >= 2 && spread <= 1 && avg >= 8) {
          txt += ' Все аяты прочитаны ровно — это признак твёрдого навыка, ' +
                 'а не случайного угадывания.';
        } else if (grades.length >= 2 && spread <= 1 && avg < 6) {
          txt += ' Все аяты дались одинаково трудно — значит дело не в отдельном аяте, ' +
                 'а в самом чтении.';
        }

        /* Теория и чтение расходятся у всех по-разному, и это самое
           личное, что можно сказать: одному нужно читать вслух, другому —
           вернуться к правилам. */
        if (qShare != null && qAll >= 4) {
          const rd = avg / 10;
          if (rd - qShare >= 0.25) {
            txt += ' Читаешь заметно лучше, чем называешь правила: слух опережает знание, ' +
                   'стоит подтянуть теорию.';
          } else if (qShare - rd >= 0.25) {
            txt += ' Правила знаешь лучше, чем читаешь: теория опережает язык, ' +
                   'помогает читать вслух каждый день, хотя бы понемногу.';
          }
        }
    ...
    /* Сильное называем ПРАВИЛАМИ, а не блоками нулевого курса. «Межзубные
       буквы держатся твёрдо» верно, но это узнавание буквы, и у половины
       класса выйдет то же самое. Правило таджвида — куда более личная
       примета. Блоки нулевого курса берём, только если правил не набралось. */
    const strongRules = [], strongC0 = [];
    Object.keys(byRule).forEach(function (t) {
      const v = byRule[t];
      if (v.all < 2 || v.got !== v.all) return;
      const g = groupOf(t);
      const gs = (g && byGroup[g].all) ? byGroup[g].got / byGroup[g].all : 1;
      if (gs >= 0.9) return;                       // раздел и так хорош — не примета
      (g === 'course0' ? strongC0 : strongRules).push(ruleName(t));
    });
    const strong = strongRules.length ? strongRules : strongC0;
    if (qShare === 1 && qAll >= 4) {
      lines.push({ mark: '✓', color: OK,
        text: '<b>В вопросах нет ни одной ошибки.</b> Правила названы верно все до одного.' });
    } else if (strong.length) {
      lines.push({ mark: '✓', color: OK,
        text: '<b>Держится твёрдо:</b> ' + strong.slice(0, 3).map(esc).join(', ') +
              (strongRules.length
                ? ' — здесь ошибок нет, хотя раздел в целом даётся не полностью.'
                : ' — эти буквы и знаки узнаются уверенно.') });
    }
